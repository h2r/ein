...
$k$ will continue to be pulled indefinitely.)  
Pseudo-code appears in Algorithm~\ref{alg:bandit2}.  We need to estimate the 
probability that the true payout probability, $\mu$, is greater than the threshold, 
$c$, given the observed number of successes and failures:
\begin{align}
\Pr(\mu_i > c|  S, F)
\end{align}
...
\While {$true$}{
  ...
  \uIf{$p_{above} \ge  \delta_{accept}$}{
    ...
    return $j$ \tcp*[r]{accept this arm}
  }\uElseIf{$totalTries \ge maxTries$}{
...
for comparison to baselines, which would be prohibitively time-consuming
to run on the real robot.  Next we describe our robotic evaluation,
which assesses the degree to which our system adaptively improves its ability
...
performance at picking using the learned grasp location and orientation.  These
...
performing objects was failure to accurately determine the position
...
robotics~\citep{brooks83, lozano89}.  These systems relied on
...
leverage existing strategies for inferring the best arm.
...
identifying the best grasp point by actively experimenting with objects
in the world.
...
a prohibitively expensive operation on our robot.  Instead our
