...
\stnote{Need a paragraph about the IR scan, the linear filter, the
  basic grasping model (discrete 2d positions and four discrete
  orientations)}

This pipeline works well on some objects, but often fails for a
variety of reasons.  For example, our grasp model is not able to find
reliable grasps for objects that are not visible in IR, such as
transparent objects or very dark objects.  Other objects need to be
...
\stnote{ This section is too short and choppy.  I'd move parts of it
  into the evaluation section (Robotic Evaluation), and other parts
  into Section 2(e.g., IR and linear filter)}
...
\stnote{ I think the policy is wrong.  I know it moves on after one,
  and accepts after 3.  But the policy says we move on after two.  I
  thought it was one before, but I misread.  Then I thought it was the
  prior, but if you add the prior, it takes more than three to accept.}
