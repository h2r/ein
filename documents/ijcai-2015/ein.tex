...
for grasp inference.  Figure~\ref{fig:ir} shows the origianl object
...
with a completely uninformed prior taking a confidence-based approach
...
The system initially uses analytic approaches to generate a grasp
hypothesis space which allows it to pick previously unencountered
objects. It then uses our bandit-based method to try new grasps and
learn instance-based distributions for the grasp experience database.
In this way our system achieves the best of both approaches: models
for grasping unknown objects can be applied; when they do fail, the
system can attempt to recover by trying grasps and adapting itself
