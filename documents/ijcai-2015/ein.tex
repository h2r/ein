...
application, but do so with very high reliability.The contribution of
this paper is to automate this adaptation process by formalizing a
grasping system as a hierarchy of bandit problems, enabling the robot
to apply learning techniques such as Thompson sampling to adapt itself
to manipulate specific objects with increased accuracy.  We define a
new variant of the bandit problem, which we term simple-regret
stopping bandits and demonstrate that it finds an optimal arm more
quickly than Thomspon sampling when we provide an ordering on arms.
Our the bandit-based adaptation step significantly improves accuracy
over a non-adaptive system, enabling a robot to quickly and
autonomously acquire models for objects, and adaptively improve them
through experience.
...
minimize simple regret after a finite exploration period.  This
approach is similar to \citet{bubeck09}.  However, in contrast to
their approach, where the agent always uses its entire training
budget, in our problem, we would like the robot to stop training once
it has acquired a good enough model for the object (for example, a
success rate greater than 95\%, so that it can move on to the next
object.  For some objects, where many grasps are effective, this
success rate can be quickly achieved with little adaptation, but other
objects require extensive trials to find a good grasp.  Our approach
enables the robot to quickly converge when it finds a good grasp, so
it can move on to the next object.
...
\begin{algorithm}
\SetKwFunction{HaltingBandit}{HaltingBandit}
\HaltingBandit{$armOrder$, $\delta$, $k$}
...
\caption{Algorithm for our Halting Bandit}
\end{algorithm}
