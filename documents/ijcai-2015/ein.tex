...
\newcommand{\algorithmDTxt}{Reckless Ordered Confidence Bound }
\newcommand{\algorithmDFname}{RecklessOrderedConfidenceBound }
...
information, enabling it to quickly find a near optimal arm without
...
1000 arms.  To address this problem, we present two new algorithms:
\algorithmCTxt, based on Hoeffding races~\citep{maron93}, and \algorithmDTxt, 
an approximation of \algorithmCTxt that explores faster and performs
similarly empiracally. In our
approaches, the robot pulls arms in an order determined by a prior,
...
the center, improving the success rate.
\begin{figure}
...
All of our detection runs on stock Baxter with one additional computer.
...
uses conventional computer vision algorithms in a simple
software architecture to achieve a frame rate of about 2Hz for object
detection and pose estimation.  Our recognition pipeline takes video
from the robot's wrist cameras, proposes a small number of candidate object bounding
boxes in each frame, and classifies each candidate bounding box as
belonging to a previously encountered object class. Object classes
...
We collect many measurements for the pointcloud by
...
employ a Parzen kernel density estimator to record z measurements at 1mm $(x,y)$
resolution. We then downsample to a $21 \times 21$ grid at 1cm resolution, which yields
s clean depth map and a tractable state space for grasp inference. We consider 
4 gripper orientations at each $(x,y)$ location in the scan for a total of 1764 possible
grasps. Our prior for grasp success is a map $(x,y,\theta) \rightarrow \mathbb{R}$ generated by convolving the 1cm depth map with
4 linear filters which correspond to the 4 orientation we consider. Their responses
are scaled linearly to be between 0 and 1 inclusive.
...
reliabley infer grasps for objects that are not visible in IR, such as
...
Our model treats grasp learning as an N-armed bandit problem. Because the
problem domain is rooted in the physical world, we can obtain high-quality
supervision at the cost of time and additional sensing by trying grasps with the
robot. Formally, the agent is given an n-armed bandit, where each arm pays
...
given a prior $\pi$ on the arms. \algorithmCTxt only makes use of order of the arms as ranked
by the prior, trying promising ones first and ignoring the specific 
values of $\pi$. \algorithmDTxt, on the other hand, actually makes use of the 
probabilities specified by the prior in order to explore more aggresively.
...
Pseudocode apperas in Algortihm~\ref{alg:bandit}.  To compute this
...
$\mu = k$ the runtime is unbounded, so we fix an $\epsilon > 0$ and
accept a grasp if $\mu \in [k-\epsilon, k+\epsilon]$ with probability
$c$.  This formalization gives rise to our policy, \algorithmCTxt,
which appears in Algorithm~\ref{alg:bandit}.
...
before eachieving a high confidence.  It almost always terminates with
...
taks many fewer trials on average, but sometimes terminates with a
...
\bottomrule
\end{tabular}
\caption{Results from the robotic evaluation. We tested on 30 objects. We use
...
reduced runtime complexity over small constants because large numbers of trials
...
sophisticated imaging equipment such as the Kinect 2. Better and faster pointclouds acquisition 
