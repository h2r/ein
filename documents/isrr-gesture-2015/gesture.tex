...
\begin{figure}
\centering
\includegraphics[width=0.3\linewidth]{figures/baxter_scene_cropped.jpg}
...
\begin{figure}
\centering
\fbox{\includegraphics{bigram-crop.pdf}}
\caption{Graphical model for our approach.\label{fig:model}}
\end{figure}
...
true object, the probabilities of the user pointing at that object with there left hand and right hand are independent:}
p(z_t | x_t) &= p(l | x_t) \times p(r | x_t) \times p(h | x_t) \times p(s | x_t)
\end{align}

The graphical model for our approach appears in
...
stood in front of a table with varying numbers of objects placed randomly on a table
Figure~\ref{fig:corpus_scene}.  We instructed subjects to pick an object and ask for it in the most natural way possible, using whatever
combination of gesture and language they felt was appropriate. The test subjects wore a microphone to pick up high-quality audio, and
we
used the HTML5 Speech Recognition package in conjunction with Google
Chrome to transcibe the speech.  This package reports incremental output
as recognition proceeds, and we perform a model update each time a new
word is perceived.  We used several object setups, using 4, 8, and 16 objects.
We used $N$ subjects, and each subject participated
in one trial per object set for a total of $3N$ trials.
...
\begin{table}
\caption{Real-world Results (4 objects)\label{table:real_results_4}}
\centering
\begin{tabular}{lr}
\toprule
Random & 25\%\\
