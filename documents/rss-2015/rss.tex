...
\jgonote{Rather than cut I decided to move it.}
...
collaborations with humans. \stnote{s/It is powerful because it/Our system} \jgonote{ACK}
...
applications of the algorithm to the tasks mentioned above.
The first application is recognizing the category of an
object and the second application is estimating the pose of the object, both of which we 
accomplish with simple and robust computer vision algorithms. The third
application is grasping the object, which we accomplish with visual servoing techniques. Each 
...
successfully and quickly pick and place the target object.  \jgonote{Quickly because
perhaps the robot will tune its own PID controller. I will write under the assumption
that it will and we can cut it if we don't get around to it.}

Crucially, our algorithm can recognize when it is doing a poor job at learning and
...
In this section we discuss each of the tasks our system tackles, both at the level of sophistication 
we are using and at the state of the art. We address typical expert training and automated approaches that exist, 
together with usability concerns.

\subsection{Object Recognition}
\jgonote{Cover BING, SIFT, BoW, typical training pipelines, and RGB-D approaches popular in the robotics
community.} Our recognition pipeline takes RGB-D video from the robot, proposes a small number of candidate 
object bounding boxes in each frame, and classifies each candidate bounding box as belonging to a previously
encountered object class. Our object classes consist of object instances rather than pure object categories.

To generate condidate bounding boxes, we first apply the BING objectness detector ~\citep{} to the image, which
...
regions in the map $O(p)$, which correspond with high probability to objects in the image. We then form
...
\subsection{Object Categorization and Pose Estimation}
For object categorization and pose estimation, we constructed data
