...
which are suceptable to local minima and take time to converge.

To address these issues, we present an approach for enabling a robot
to train its own view-based method to recognize and manipulate the
specific objects it will need to use during future collaborations with
humans.  View-based methods have many advantages over model-based
methods, because they directly capture the appearance of the object,
directly operate on low-level features, and are relatively simple and
efficient to implement~\citep{hsiao13}.  However these systems require
large amounts of training data for robust performance, for example
more than 2000 objects as reported in \citet{hsiao13}.  We address
this problem by enabling a robot to learn to identify and grasp on a
per object basis by autonomously collecting large amounts of
supervised training data for instance-based visual detection and pose
estimation.  Our contribution is an algorithm which allows a robot to
autonomously train its subsystems by using slower, more expensive
sensing approaches to provide supervision for faster, simpler methods
that excel with large amounts of training data.
...
camera as a seven-degree of freedom one-pixel RGB-D camera.  Our
approach simultaneously acquires a 3d model of the object, traings
visual detectors and pose estimation models that can be used for
active visual servoing for grasping, as well as for semantic mapping
on a mobile robot.  Because our camera can move with seven degrees of
freedom, the robot can collect large quantities of data leading to
very simple visual models that perform with very high accuracy under
occlusion.
...
grasping.  Existing approaches for instance-based grasping use on the
order of $25$ images~\citep{hsiao13}
...
\begin{figure}
  ...
  \begin{algorithmic}
    ...
    \ENDWHILE
  \end{algorithmic}
  \caption{The high-level object-learning algorithm.}
\end{figure}
