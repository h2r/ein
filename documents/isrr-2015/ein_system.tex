...
\abstract{A key aim of current research is to create robots that can
  ...
  instance based models for detecting the object, estimating its pose,
  and grasping the object.  This approach achieves the generality of
  category-based methods with the reliability of instance-based
  methods, at the cost of the robot's time collecting data for the
  object models. We demonstrate that our approach enables an
  unmodified Baxter robot to autonomously acquire models for a wide
  variety of objects and then detect the objects on the tabletop and
  respond to pick-and-place requests.  }
...
and accuracy.  A visualization of data flow in the detection and
localization pipeline appears in Figure~\ref{fig:system}.

\begin{figure}
\includegraphics{figures/system.pdf}
\caption{Results at each phase of the localization pipeline.\label{fig:system}}
\end{figure}
...
\subfigure[Depth map.\label{fig:depth_map}]{\includegraphics[width=0.4\linewidth]{figures/ruler_06_depth.png}}
\subfigure[\label{fig:gradient}Gradient map.]{\includegraphics[width=0.4\linewidth]{figures/ruler_07a_aerialGradient.png}}

\caption{Autonomously acquired object model.\label{fig:ruler_model}}
\end{figure}
...
\begin{figure}
\subfigure[Tabletop scene with objects]{
\includegraphics[width=0.5\linewidth]{figures/mapping/mergedMapScene.png}}
\subfigure[Map created for the scene by scanning with both arms.]{
\includegraphics[width=0.5\linewidth]{figures/mapping/mergedMap.png}}
...
of the robot's capability to lift.  In our other
...
\begin{figure}
\subfigure[The depth map for the glass bowl without a contrast agent applied is noisy and leads to poor grasp hypothesis.]{\includegraphics[width=0.5\linewidth]{figures/glassBowl.png}
\includegraphics[width=0.5\linewidth]{figures/glassBowlIrNoContrast.png}
}
\subfigure[When a contrast agent is applied, the depth map is clearn and grasp performance improves.  In our method, the contrast agent is only needed at scan time, not at pick time.]{
\includegraphics[width=0.5\linewidth]{figures/glassBowlContrast.png}
\includegraphics[width=0.5\linewidth]{figures/glassBowlIrContrast.png}}

\caption{Depth map for a transparent object with and without a contrast agent.\label{fig:contrast}}
\end{figure}

Many objects are challenging to grasp for our approach because they
are transparent or dark in IR; grasping objects in spite of these
issues is a challenging problem that remains an active area of
research~\citep{ihrke08, meriaudeau12}.  As in \citet{lysenkov13},
which used paint to construct models of objects, we apply a contrast
agent to make the object visible in IR.  Because our approach only
uses depth information at scan time, and not at grasping time, and is
heavily instance-based, we apply a temporary coating for the IR scan
and then remove it for grasp training.  Figure~\ref{fig:contrast}
shows an object and IR scan with and without a contrast agent.  This
demonstrates the advantage of our view-based and instance-based
...
Other objects, such as those in
Figure~\ref{fig:poor_ir} are poorly visible in IR.  
