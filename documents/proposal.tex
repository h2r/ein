...
Our current framework for detection uses a box metaphor to talk about space in a way that 
...
We determine Green Boxes using an Objectness measure \cite{bing}, extract features with SIFT \cite{sift} 
and k-means, and perform classification on blue and red boxes using k-nearest neighbors.

Right now, detections and pose estimates are published in ROS for Blue and Red Boxes. To provide accurate help
...
The Red Box optimization process can be framed as an MDP \cite{MDP}. We might achieve better performance
by using more sophisticated techniques that incorporate feedback with the robot's movement planner in 
order to collect additional, specific data as it is needed.  
Using a POMDP \cite{POMDP} instead of an MDP is a natural approach to investigate.
