...
//	  hiColorRangemapImage.at<cv::Vec3b>(px,py) = cv::Vec3b(tBlue, tGreen, tRed);
  ...
  if (shouldIRender) {
	...
	cv::imshow(aerialGradientViewerName, aerialGradientViewerImage);
      }
    }
  }
  #ifdef DEBUG
...
//line(Mat& img, Point pt1, Point pt2, const Scalar& color, int thickness=1, int lineType=8, int shift=0)
	...
	// ATTN 18 
	// careful with the flush factor
	// negative currentTableZ because it was stored sign flipped
...
//	}
    ...
    break;
    // set random position and orientation for bblearn
    // capslock + numlock + '
    case 1179687:
      {
	...
	currentEEPose.oz += noTheta;
      ...
      break;
    // set random orientation for photospin.
    // capslock + numlock + "
    case 1310722:
      {
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg){
  ...
  if (densityViewerImage.rows <= 1 || densityViewerImage.rows <= 1) {
