...
bool ikfast_search(MachineState * ms, geometry_msgs::Pose pose, double free, std::vector<double>& outsol)  {
  ...
  vector< vector<double> > allsolutions;

  int p_num_threads = 8;
  vector< vector< vector<double> > > threadsolutions;
  threadsolutions.resize(p_num_threads);

  #pragma omp parallel for
  for (int thread = 0; thread < p_num_threads; thread++) {
	...
	threadsolutions[thread].push_back(solution);
      }
    }
  }
  ...
  int totalnumsol = 0;
  for (int thread = 0; thread < p_num_threads; thread++) {
    totalnumsol += threadsolutions[thread].size();
  }
  allsolutions.reserve(totalnumsol);
  for (int thread = 0; thread < p_num_threads; thread++) {
    allsolutions.insert( allsolutions.end(), threadsolutions[thread].begin(), threadsolutions[thread].end() );
  }

  for (int s = 0; s < totalnumsol; s++) {
    vector<double> solution = allsolutions[s];
